...
def merge_sections(term: Term, banner_sections: list[BannerSection]):
    ...
    old_professors_by_email = { professor.email:professor for professor in session.scalars(select_prof).all() }
        ...
        for banner_meeting in banner_section.meetings:
            ...
            for day, is_day in day_is_day:
                ...
                if bt is not None and et is not None:
                    start_time = int(bt[0:3]) * 60 + int(bt[3:])
                    end_time = int(et[0:3]) * 60 + int(et[3:])
                meeting_dict = {
    ...
    update_prof = sqlite_insert(Professor).values(
            ...
            [{ p1:p2 for p1, p2 in professor.as_dict().items() if p2 is not None and p1 != "email"} for professor in new_professors]
            )
