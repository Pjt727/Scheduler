...
class UpdateMeetingsContext(TypedDict):
    ...
    building: str | None
    edit_room: str | None
    section_pk_to_intersection_group: dict
...
def get_update_meeting_context(
    edit_meetings: list[EditMeeting] | None = None,
    building: Building | None = None,
    room: Room | None = None,
    duration: timedelta | None = None,
    other_meetings: QuerySet[Meeting] | None = None,
    open_slots: list[TimeSlot] | None = None,
) -> UpdateMeetingsContext:
    all_meetings: list[EditMeeting] = []
    if edit_meetings:
        all_meetings.extend(edit_meetings)
    if other_meetings:
        all_meetings.extend(
            map(lambda m: EditMeeting.from_meeting(m, 0), other_meetings.all())
        )
    intersection_groups = EditMeeting.get_intersection_groups(all_meetings)
    section_pk_to_intersection_group = {}
    for i, intersection_group in enumerate(intersection_groups):
        if len(intersection_group) <= 1:
            continue
        for meeting in intersection_group:
            # print(meeting)
            section_pk_to_intersection_group[meeting.section.pk] = i
        # print("\n\n\n")
        # print("---new group---")
        # print("\n\n\n")
    ...
    context: UpdateMeetingsContext = {
        ...
        "section_pk_to_intersection_group": section_pk_to_intersection_group,
    }
    return context


# Please refactor there is too many redundant if's
def generate_update_meeting_context(
    data: QueryDict,
    edit_meetings: list[EditMeeting] | None = None,
    edit_meeting: EditMeeting | None = None,
    added_section: Section | None = None,
) -> UpdateMeetingsContext:
    ...
    other_meetings = None
    open_slots = None
    conflicting_courses = Course.objects.filter(pk__in=data.getlist("course"))
    ...
    other_meetings, open_slots = EditMeeting.get_open_slots(
        ...
        conflicting_courses=set(conflicting_courses.all()),
        duration=duration,
    )
...
class DisplayRow(View):
    def delete(self, request: HttpRequest) -> HttpResponse:
        ...
        display_meeting_context = {"edit_meeting": edit_meeting}
...
class InputRow(View):
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        data = QueryDict(request.body)  # pyright: ignore
        edit_meetings, edit_meeting = EditMeeting.create_all(data)
        display_meeting_context = {"editing_meeting": edit_meeting}
        update_meeting_context = generate_update_meeting_context(data, edit_meetings)
        context = update_meeting_context | display_meeting_context
        return render(
            request, "input_row.html", context=context | update_meeting_context
        )
...
def update_time_intervals(request: HttpRequest) -> HttpResponse:
    ...
    context = {"time_intervals": time_intervals, "start_time": current_start_time}
...
def update_duration(request: HttpRequest) -> HttpResponse:
    ...
    time_intervals = TimeBlock.get_time_intervals(
        edit_meeting.duration, edit_meeting.day
    )

    start_time_in_interval = any(
        map(lambda i: i[0] == edit_meeting.start_time, time_intervals)
    )
    ...
    page_context = {"time_intervals": time_intervals, "start_time": start_time}

    update_meeting_context = generate_update_meeting_context(
        data, edit_meetings, edit_meeting
    )
...
def add_rows(request: HttpRequest) -> HttpResponse:
    ...
    recommended = EditMeeting.recommend_meetings(
        edit_meetings, section.primary_professor, section
    )
...
def add_section(request: HttpRequest) -> HttpResponse:
    ...
    update_meeting_context = generate_update_meeting_context(
        data, edit_meetings, added_section=section
    )
...
def soft_submit(request: HttpRequest) -> HttpResponse:
    ...
    for section, meetings in section_edit_meetings.items():
        section_problems.append(
            (
                section,
                EditMeeting.get_section_problems(
                    meetings, sections_to_exclude=sections_to_exclude
                ),
            )
        )
    ...
    context = {
        ...
        "exclude_group_problems": (len(sections_to_exclude) == 1)
        and (len(group_problems) == 0),
    }
...
def hard_submit(request: HttpRequest) -> HttpResponse:
    ...
    message_bundle = EditMeetingMessageBundleRequest(
        message=message, requester=professor, request=bundle
    )
    message_bundle.save()
...
def soft_approve(request: HttpRequest) -> HttpResponse:
    ...
    for edit_section in request_bundle.request.edit_sections.all():
        ...
        for i, edit_meeting_request in enumerate(
            edit_section.edit_meetings.all(), start=1
        ):
...
def hard_approve(request: HttpRequest) -> HttpResponse:
    ...
    messages.success(
        request, ("Successfully approved the request"), extra_tags="success"
    )
    return HttpResponseClientRedirect(reverse("message_hub"))
...
def cancel_request(request: HttpRequest) -> HttpResponse:
    ...
    if professor != request_bundle.requester:
        return HttpResponseForbidden(
            "You did make the request so you cannot cancel it!"
        )

    request_bundle.delete()
    messages.success(
        request, ("Successfully cancelled the request"), extra_tags="success"
    )
