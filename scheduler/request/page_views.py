...
def edit_section(request: HttpRequest, section_pk: int) -> HttpResponse:
    ...
    edit_section = (
        EditSectionRequest.objects.filter(section=section)
    ...
    other_meetings, open_slots = EditMeeting.get_open_slots(
        term=section.term,
        conflicting_courses=set(),  # TODO change to ones added by tags
...
def message_hub(request: HttpRequest) -> HttpResponse:
    ...
    current_bundles = professor.requested_bundles.filter(response__isnull=True).exclude(
        response__is_read=True
    )

    current_bundles_sorted = current_bundles.annotate(
        max_date=Max("date_sent", "response__date_sent")
    ).order_by("-max_date")

    # sorting does not work as expected
    past_bundles = (
        professor.requested_bundles.exclude(pk__in=current_bundles)
